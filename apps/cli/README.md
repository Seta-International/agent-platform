...
| `seta-cli seed` | Seed the SETA International tenant + admin and the full cross-module fixture from the gitignored `fixtures/seta/seta-fixture.xlsx` workbook (auto-creates the tenant + admin; degrades to tenant + admin only when the workbook is absent; idempotent) |

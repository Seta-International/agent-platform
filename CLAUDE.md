...
1. **`pnpm depcruise`** — cross-module imports must go through `packages/<module>/src/index.ts` or the `/events`, `/rbac`, `/contracts`, `/agent-tools` subpaths. `shared-*` may not import from feature modules. `agent` is engine-only and may not import any feature or orchestrator module (`agent-no-feature-imports`). Frontend app tier: `no-cross-web-app-imports` — leaf `web-*` apps can't import each other (`web-identity`/`web-notifications`/`web-agent`'s Ask Seta panel are importable infra; cross-app composition only in the `apps/web` shell); `web-no-backend-imports` — no web package imports a module's `/backend` or `/db`. Database access: `shared-db-composition-root-only` — `@seta/shared-db/composition` (`getPool`) is importable only by `apps/{server,worker,cli}`; module code resolves `executorPool()` instead. `shared-db-pre-tenant-identity-only` — `@seta/shared-db/pre-tenant` is importable only by `packages/identity/src/backend/`.
...
5. **The database constitution** — eighteen rules asserted against `pg_catalog` *after* the migrations run, in `apps/cli/tests/integration/db-constitution.test.ts` (the only place all ten modules' migrations meet one database). It sees tables born in hand-written SQL, which a `schema.ts` lint cannot. A new table satisfies the rules or earns a row in `scripts/lint/db-constitution-baseline.json`: `accepted` needs a reason, `debt` needs a reason and a `FUT-<n>`. Both directions ratchet — a fresh violation fails, and a row that no longer violates must be deleted. `pnpm db:constitution` regenerates the file with empty reasons, which do not parse; you cannot generate your way past the gate.
...
**The composition root chooses privilege, not module code.** A module's `xDb()` resolves `executorPool()`, which reads an `AsyncLocalStorage` context and **throws `ExecutorContextError` outside one**. Entry points open it: `sessionMiddleware` → `scoped(tenantId)` (the `seta_app` role, `NOBYPASSRLS`, `app.tenant_id` bound — this is what makes RLS enforce); `wrapJob` → `scoped()` or `maintenance()`; `apps/cli` → `maintenance()` (the admin role, `BYPASSRLS`). Never silence an `ExecutorContextError` by reaching for a raw pool — if code genuinely cannot know its tenant, that is a design question, not an import. `maintenance()` is a privilege grant and is countable: six in production, each justified at its call site. Tests enter the context production enters — seed helpers `maintenance()`, test bodies `scoped(session.tenant_id, ...)`. **Never give a shared test harness an ambient context**; a missing wrapper must fail loudly. A db handle is resolved per call, never captured: a factory takes `db: () => xDb()`, not `db: xDb()`.

...
- **Cross-module identity:** the stable cross-module `worker_id` **is `person.id`** (unchanged across re-hires); `rm_allocation`, `pm` assignments, and visibility are all keyed on `person.id`. The directory row derives from a **`v_worker_directory` view** (open `employment_period` + current comp/capacity + held position); domain fields (lifecycle_stage/grade/fte/department/role_title) are **not cached on `worker`** — this removes the dual-write drift the review flagged. A re-hire just opens a new `employment_period`; identity/dob/gender persist on `worker`.
...
- `person` carries the identity link; idx `(tenant_id, user_id)` (the `user_id` lookup is on **`person`**, not `worker`).
- `worker` *(person-level mutable directory fields only)* — `person_id` (unique FK), `full_name`, `work_email`, `location`, `gender`, `dob`, `phone`, `emergency_contact jsonb`, `profile_completed_at`, `version`, `deleted_at`. **GIN-trigram** `(full_name, work_email)`. Derived domain fields (lifecycle_stage/status/grade/fte/department/role_title) come from the **`v_worker_directory` view** (open `employment_period` + current comp/capacity + held position) — no cached domain columns, so no dual-write drift.
- Added invariants (post-challenge): `employment_period unique(person_id, seq)`; `position` partial-unique `(tenant_id, holder_worker_id) WHERE holder_worker_id IS NOT NULL` (a worker holds ≤1 seat); `org_unit` **acyclicity constraint trigger** (was domain-only); `rm_allocation unique(tenant_id, allocation_id)` (idempotent projection — closes the RBAC double-count); `rm_allocation.bucket (billable|internal|bench)`; `document_requirement.employment_type` (binds the by-type scope); CHECK enums on lifecycle/status/movement/health columns.
...
## Appendix H — v3 hardening (research-grounded; see `docs/spike/review-schema.sql`)

Validated against PostgreSQL docs, SQL:2011 temporal, Kleppmann/Fowler (CQRS read models, event sourcing), AWS/Citus multi-tenant, and Workday/Greenhouse/Float reference models. Applied + verified on the standalone review DB.

- **Temporal integrity:** `unique(entity, effective_from)` allows *overlapping* periods → add a GiST **`EXCLUDE`** over `daterange` (`btree_gist`) on `worker_compensation`/`worker_capacity` (and `pm.rate`); keep the partial-unique for one-open row. Migrate to native `WITHOUT OVERLAPS` keys on PG18.
- **Audit:** the outbox (`core.events`) is *intent*, not a field-level audit. Add **one generic trigger audit table** (`core.audit_log`, supa_audit jsonb shape) with `actor_id` from a `SET LOCAL` GUC, `REVOKE UPDATE,DELETE`, BRIN(ts); attach to business tables. Reserve `*_event` tables for domain timelines. Added decision-actor columns the outbox can't substitute for (`probation_review.decided_by`, `account_access_grant.revoked_by`, `movement_request.decided_by/rejected_reason`).
- **Read/write (directory):** the live `v_worker_directory` join measured 117 ms / 22k buffers for 50 rows at 20k workers (filter applied after the join+sort — no LIMIT pushdown). Replace with an **event-maintained `rm_worker_directory` projection** (drift-safe because event-sourced, not app-dual-written — the same discipline as `rm_allocation`), indexed `(tenant_id, lifecycle_stage, full_name)` + trigram. Added `lifecycle_case_step` (step-duration timeline), `rm_workforce_metrics`, and ~14 hot-path indexes; BRIN on append-only tables; defer range partitioning to pg_partman at RAM-scale.
- **Evidence rules enforced in-DB:** `review_score` CHECKs — extreme score (1/5) requires `evidence`; `<4` requires a `top_action`.
- **Multi-tenant/RLS:** the app role must be **non-owner / non-BYPASSRLS** (or `FORCE ROW LEVEL SECURITY`); set tenant context as **`SET LOCAL app.current_tenant`** (pooler-safe); keep the RLS predicate leakproof (`tenant_id = current_setting(...)::uuid`) so it pushes below joins; lead composite indexes with `tenant_id`.

...
`requisition` (`status open|on_hold|filled|cancelled`, `stage`, `resource_request_id null`, `position_id null`, `kind replacement|new`, `jd jsonb`, `owner_user_id`, `closed_at`); `candidate` (`source`, `contact jsonb`, `dob`, `gender`, `cv_storage_key`, `status`, `stage`, `skills`, `seniority`, **`segment` (incl. alumni)**, `reject_reason`, `tags`); `application` (`requisition_id`, `worker_id`, `status submitted|releasing_endorsed|receiving_endorsed|pmo_review|approved|rejected|withdrawn`, `alloc_pct`, `override_overallocation`, **`mobility_event_id` guard**); `application_event` (endorsement history); `interview` (`candidate_id?`/`application_id?`, `round`, `panel jsonb`, `mode`, `meeting_link`, `status scheduled|completed|cancelled|no_show`, `result pass|hold|fail`, `rating`, `recommendation`, `transcript`, **`scorecard_template_id` + criteria snapshot (immutable copy)**); `interview_score` (normalized, by `criterion_id`); `offer` (`status draft|approved|sent|accepted|declined|expired`, `respond_by`, `hired_event_id` guard, partial uniq accepted-per-candidate); `resource_request_fulfillment` (saga); `kb_article`.

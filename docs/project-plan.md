...
Spec + per-slice plan files: _internal design notes_.
...
| D16 | Dispatcher hardening: per-`(subscription, event_id)` exponential backoff (1s→60s, 5 attempts) → DLQ + cursor advance; multi-replica safety via `FOR UPDATE SKIP LOCKED` on cursor row | _internal design notes_ §3.3 |
...
| D21 | `packages/shared-ui` is the style monopoly; tokens live only in its `@theme` block; shadcn primitives are overridden to reference DESIGN.md utilities directly (no alias bridge); CI grep audit (`pnpm lint:styles`) blocks any `.css`, `tailwind.config.*`, or `@theme/@layer/@apply` outside that package | _internal design notes_ §3 + §7.3 |
| D22 | DataTable Layer-1 composite ships with client and server modes — sort / filter / global search / expand / select / pagination / column visibility / loading / empty; built on `@tanstack/react-table` + shadcn Table primitive; resizing, drag-reorder, sticky columns, inline edit, virtualization deferred | _internal design notes_ §5 |
| D23 | Chromatic accent re-anchored from Linear lavender (`#5e6ad2`) to Seta blue (`#0047FF` / hover `#1A3CFF` / focus `#022DAD`); palette sourced from `seta-international.com` site CSS to match the brand mark; `DESIGN.md §Colors.Brand & Accent` follow-up PR updates the spec text to match | _internal design notes_ §3.1 + §8.4 |
| D24 | M2 Stream A split into A1 (local-password) + A2 (Entra OIDC). Both required for M2 Stream A demo gate. Rationale: scope tractability; A1 sets forward-compat seams A2 builds on. Reversal: both slices must close before M2 is considered done. | _internal design notes_ |
| D25 | No internal contract versioning. Event payloads / TS types mutate in place; `event_version` column stays as a forward hook but never bumps. External URL versioning (`/api/v1/...`) unchanged. Rationale: single-deploy modular monolith; in-process subscribers; no external consumers. Reversal: first external system (other process, public webhook) consuming events — start versioning then. | _internal design notes_ |
| D26 | Provider-subject mapping lives in better-auth's `identity.account` (`providerId`/`accountId`). Per-tenant SSO config lives in `identity.tenant_sso_providers` row-per-provider. Rationale: `requirements.md §3.2` mandates additional providers (Google, Okta, generic SAML/OIDC) without schema change. Reversal: if a provider's metadata cannot fit the `account` row + jsonb config, revisit. | _internal design notes_ |
| D27 | Admin/CLI-created users skip better-auth's `signUp.emailAndPassword`. Compose `user`/`account`/`user_profile`/`role_grant` rows inside `withEmit()` directly. Rationale: better-auth's signUp wants email verification (mailer-dependent) + session cookie; both wrong for admin/CLI. Reversal: when `shared/mailer` lands (M3) and we add public sign-up — revisit. | _internal design notes_ |
| D28 | HIBP outage allows password set/change with logged warning + OTel counter. Rationale: blocking on HIBP availability reduces production-safety, not increases it. Reversal: outage rate >1%/quarter → cache k-anonymity responses or ship a local breach list. | _internal design notes_ |
| D29 | M2 Stream A · A2: JIT removed; admin pre-provisioning + account linking only. Rationale: tighter admin control over access; user-instructed. Deviates from `requirements.md §3.4`. Reversal: re-enable JIT only if a customer pushes back hard and we ship a follow-up "JIT mode" toggle per tenant. | _internal design notes_ |
| D30 | Single multi-tenant Entra app registration owned by Seta operator. Rationale: lowest-friction operator UX; data isolation via `tenant_id` + `tid` validation, not via app registration. Reversal: per-tenant own Entra app deferred to v1.x for high-trust enterprise tenants needing credential isolation. | _internal design notes_ |
| D31 | Email domain ownership verified transitively via Microsoft Graph `Domain.Read.All`. Rationale: inherit Entra's existing DNS verification; no DNS plumbing on Seta side. Reversal: add DNS TXT verification later if Microsoft Graph ever stops exposing isVerified or rate-limits. | _internal design notes_ |
| D32 | Admin consent flow is two-step Connect→Consent→Enable. `consent_granted_by_*` populated lazily. Rationale: Microsoft's admin-consent endpoint does not return user claims; two-step is the production-grade norm. Reversal: eagerly capture on a future Microsoft endpoint update. | _internal design notes_ |
| D33 | IdP group→role mapping deferred to Phase B "SSO group sync" slice. Rationale: A2 ships pure auth + linking; role grants stay admin-driven. Reversal: triggered when a customer with strict IdP discipline asks for it. | _internal design notes_ |
| D34 | Single image, mode-selectable runtime. The "one runtime binary" framing in `architecture.md §A` is amended to "single image, mode-selectable runtime; defaults to all-in-one." `SETA_MODULES` env var selects loaded modules at startup. Cross-module sync calls route through a dispatch shim (`packages/core/src/rpc/`) that picks in-process call vs Hono RPC client based on the loaded-module set. Event bus and boundary discipline unchanged. Rationale: enables independent per-module scaling on AWS ECS Fargate without forking the codebase into microservices; preserves single-container OSS self-host as the default. Reversal: nothing reverses. Supersedes the implicit single-process assumption in §A. | _internal design notes_ |
| D37 | `shared/crypto` envelope format: AES-256-GCM, per-message DEK, JCE-style JSON blob `{ v, alg, kid, wdk, iv, ct, tag }` with `v=1`. Two `KeyProvider` impls (`KmsKeyProvider`, `EnvKeyProvider`); `CRYPTO_KEY_PROVIDER` env selects. Pulled forward from M3 to M2-late because `shared/mailer` depends on it. `selfTest()` at boot crash-loops the process on misconfig. Format supports rotation (`kid` per blob) but does not execute it; active re-encrypt remains Phase B per D11c. | _internal design notes_ |
| D36 | `shared/mailer` adopts tenant-first / operator-fallback transport resolution. Per-tenant config (Graph or SMTP) in `integrations.mail_transport_config`; operator default (SMTP-via-nodemailer or dev-stub) in env. Templates centralised in `packages/shared-mailer` (`react-email` TSX). Send queued via graphile-worker; `core.outgoing_emails` is outbox + dedupe + audit. Per-tenant SMTP passwords encrypted via `shared/crypto`. Graph reuses the operator's existing multi-tenant Entra app — adds `Mail.Send` (application) and requires Entra SSO configured per tenant + Application Access Policy documented. Delta from D13/§J.9. | _internal design notes_ |
| D27-rev | Reversal of D27: admin/CLI user creation can now trigger an invite email via `shared/mailer` when a `CreateUserInviteOpts` is supplied. The invite link doubles as proof of email control for pre-provisioned users; explicit verification still skipped for admin-created accounts. Existing call sites unchanged — invite is opt-in by the caller. | _internal design notes_ |
| D38 | M2 Stream B reshape: split into B1 (foundation), B2 (web UI), B3 (MS connect), B4 (two-way sync). Mastra tools + `planner.agent` deferred to post-planner phase; Stream C (copilot runtime) consequently also deferred. Rationale: user direction to clone MS Planner experience with two-way sync as M2's primary planner deliverable; scope too large for single spec/PR. Reversal: re-engage Stream C if customer commitment requires the agent demo before planner program closes. (Recorded as D34 in the source spec; renumbered here to follow the existing ledger sequence — D34 was claimed by the deployment-strategy ADR landed earlier, D35–D37 by the crypto/mailer ADRs landed since.) | _internal design notes_ |
| D39 | Planner public-surface (`packages/planner/src/index.ts`) frozen on B1 merge. Function signatures, DTO shapes, event payloads, `PlannerError` codes do not change after B1; B2/B3/B4 and any future Mastra slice import the frozen contract. Internal refactors are unaffected. Reversal: a signature change requires a new D-row explicitly retiring this freeze with the reason. (Recorded as D35 in the source spec; renumbered here for ledger continuity.) | _internal design notes_ |
| D40 | B2 (planner web UI) ships in a 3-PR split: PR1 foundation (routing + data layer + Groups/Group detail/Trash) → PR2 board+sheet (Kanban DnD + Task sheet) → PR3 grid+polish. Locks `@hello-pangea/dnd` for DnD, TanStack Query + SSE reconciliation, plain `Textarea` + `react-markdown` for the description editor (TipTap/Lexical + @mentions deferred to Phase B). One additive change to the planner public surface in PR1: `listTaskEvents` + `GET /api/planner/v1/tasks/:id/events`; D39 freeze preserved. Reversal: a different DnD library or data layer requires a new D-row retiring this lock. | _internal design notes_ |
| D41 | Split workflow `execute.self` from `approve`. Re-run is a per-user action; approval is per-run authorization. Bundling leaks approval power to anyone who can re-run an action. Recorded 2026-05-21 in the workflow runs foundation slice. | _internal design notes_ |
| D42 | Vector index tenant isolation: declarative LIST partitioning by `tenant_id` from day 1; per-partition HNSW; planner prunes at WHERE. Reverses §A10 "no partitioning in v1". Rationale: arXiv:2401.07119 (Curator) — 37× faster vs. shared-index prefilter at 100-tenant scale. | _internal design notes_ §3.2; `architecture.md` §A10 (edited) |
...
| D50 | Worker runtime split. `apps/server` (HTTP) + `apps/worker` (graphile-worker pool + LISTEN/NOTIFY dispatcher) + `apps/cli` (ops/migrations) — three runtimes sharing the composition library at `@seta/core/runtime`. Dev convenience: `apps/server` runs `startBoth()` (HTTP + worker in-process); production splits into separate ECS services. Replaces the "single Node process" assumption. Rationale: production topology requires the dispatcher to run exactly once across the fleet; convention insufficient under autoscaling. Reversal: nothing reverses. | `architecture.md` §A13 |
| D51 | Notifications module extraction. Notification platform (in-app + email prefs + SSE hub) moves from `@seta/core` to `@seta/notifications` with its own `notifications.*` schema. Rationale: notifications owns its own schema, RBAC, routes, subscribers, and SSE — every measure of a module except its location. Reversal: nothing reverses. | `architecture.md` §J.8; `packages/notifications/README.md` |
| D52 | Knowledge module extraction. File upload + parse + embed + RAG pipeline moves from `@seta/copilot` to `@seta/knowledge` with its own `knowledge.*` schema. Same reasoning as D51. Reversal: nothing reverses. | `packages/knowledge/README.md` |
| D53 | Copilot becomes engine-only. `@seta/copilot` may not import any feature or orchestrator module (enforced by dep-cruiser rule `copilot-no-feature-imports`). Agent tools are owned by modules and exposed through `reg.module({ agentTools })`; copilot consumes them via the contribution registry. Rationale: avoids copilot becoming the dependency sink of every AI-touched feature; lets `@seta/copilot-sdk` be a tiny contract package and lets modules build tools without depending on Mastra. Reversal: a feature whose tools cannot be expressed against the SDK contract — fix the SDK, not the boundary. | `architecture.md` §A5 #4–#6, §B; `sdks/copilot/` |
| D54 | Two enforced tiers. `infra` (`packages/shared-*`, `sdks/*` — leaf packages) and `module` (everything else). `foundation`, `orchestrator`, and `engine` become documented patterns (declared via `setaTier` in `package.json`), not enforced layers. Rationale: the failure modes of orchestrator/engine are already covered by `no-circular` + `no-cross-module-internals`; a third enforced tier adds maintenance without adding safety. Reversal: a recurring boundary violation that the two-tier rules can't catch — re-introduce enforcement then. | `architecture.md` §B.0; `CLAUDE.md` "Module tiers" |
| D55 | `reg.module({...})` unified registry call. Single object replaces the previous 5 separate methods (`schema`, `migrationsDir`, `subscribers`, `publicApi`, `workflows`). New optional fields: `events`, `rbac`, `routes`, `stream`, `agentTools`, `agentSpecs`, `errorMapper`. Registry validates at composition time (duplicate names, missing event schemas, RBAC slug collisions). Rationale: one call site per module simplifies the factory and gates validation; `publicApi` was dead (every caller passed `{}`) and `workflows` rides naturally inside the module contribution alongside everything else. Reversal: nothing reverses. | `architecture.md` §J.7; `packages/core/src/composition/registry.ts` |
| D56 | Flat workspace, `shared-` prefix kept. `packages/shared/<x>/` → `packages/shared-<x>/`. Prefix kept (rather than dropped in favour of a maintained allowlist) so path-based boundary enforcement requires no maintained list. Rationale: standard monorepo convention (flat) vs. cheapest enforcement mechanism (prefix) — prefix wins because Seta's modular monolith depends on the boundary rule firing without human upkeep. SDK contract packages live under repo-root `sdks/` (`@seta/copilot-sdk`, `@seta/module-sdk`). Reversal: nothing reverses. | `requirements.md` §19.1; `architecture.md` §B.6 |
| D57 | `console-web` aggregates tenant-admin UI. All admin pages (users, SSO, audit, integrations, notification prefs, tenant settings) live in `apps/web/src/modules/console/`. Other web modules don't carry their own admin sub-areas. Rationale: one package per admin domain doesn't scale; aggregation matches the user surface (admin nav is one section). Reversal: an admin domain large enough to justify its own routing tree — extract it then. | `apps/web/src/modules/console/` |
| D58 | TanStack file-based routing kept; full manifest framework deferred. Per-web-module typed `navManifest` declares nav + required permissions only. Slot extension system and code-based routing wait until either (a) the first cross-module UI extension lands or (b) the web module count reaches ~10. Rationale: full Odoo-scale framework has unclear payoff at the current 3-manifest count; pay later when demand justifies it. Reversal: trigger conditions (a) or (b) above. | `apps/web/src/shell/manifests.ts`; `sdks/module/` |

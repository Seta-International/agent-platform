...
The spike's "Step 5 — WBS (buildable slices)" tables (e.g. `docs/spike/pm.md`) are the embryonic form; this playbook turns them into an importable spreadsheet.
...
**Step 1 — Inventory the deliverables.** The **capability inventory** is your level-1 decomposition — it already exists for the discovery modules (`PM-Cn`, `H-Cn`, `P-Cn` in `docs/spike/`). For a module being extended, dispatch parallel exploration agents (domain/behavior, data model, events/integration, access control, frontend/UX), each returning facts with `file:line` cites. Synthesize a capability list; don't paste agent dumps into the CSV.

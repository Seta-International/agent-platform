...
2. **A2 receives it as data.** `renderOpenPreviewBlock` appends an OPEN PREVIEW block to the turn message — owning tool, the card's intent line, and the card's own `kvTable` rows. The DTO also carries the card's `taskIds`, which the server matches against. The machine-readable `argsPatch` never reaches the prompt, so no proposed value can be smuggled back in through model text.
3. **The SERVER decides whether this is an adjustment (design D20).** `resolveRevision` takes no input from the model: it matches the open card's `meta.toolId` against the calling tool and the card's `taskIds` against the tasks the turn resolved, then loads the row. Any mismatch — no card open, another tool's card, a different task, a row decided since the lookup — falls through to the new-card path, where the `task:` mutex refuses it in a sentence if it must. Targets then come FROM THE CARD, permissions are re-gated, the patch is merged over the previous one, and a FRESH idempotency key is minted. The model contributes only `correction: boolean` (narrowing vs adding); the server computes which fields that drops, deletes patch fields that would change nothing, and returns a pre-rendered from→to diff. **Merge's role swap is the single deliberate exception** — it may swap which task survives, never which two tasks are involved. No approval id is exposed to the model at all, which makes FUT-824's "an id quoted in hostile text buys no access" true by construction.

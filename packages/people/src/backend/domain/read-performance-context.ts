...
export async function readPerformanceContext(
  ...
  const monthStart = `${input.as_of_month}-01`;
  const inMonth = and(
  ...
  const allocRows = await db
    ...
    .where(
      and(
        ...
        or(
          eq(workerAllocationProjection.person_id, me),
          eq(workerAllocationProjection.lead_person_id, me),
        ),
        inMonth,
      ),
    );

  // One capacity per project; leading any allocation on a project makes it a TL capacity.
  const byProject = new Map<string, PerformanceCapacity>();
  for (const r of allocRows) {
    const isLead = r.lead_person_id === me;
  ...
  const amAccountIds = await listAccountIdsManagedBy(me, session.tenant_id);
  ...
  const capacities = [...amCapacities, ...byProject.values()].sort(
    (a, b) =>

...
export function registerPeopleCvRoutes(
  ...
  app.post('/api/people/v1/cv/parse-draft', async (c) => {
  ...
  app.post('/api/people/v1/workers/:id/cv/upload-url', async (c) => {
  ...
  app.get('/api/people/v1/workers/:id/cv/download-url', async (c) =>
    c.json(await workerCvDownloadUrl({ worker_id: c.req.param('id'), session: c.get('user') })),
  );
}

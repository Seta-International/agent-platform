...
describe('editWorker', () => {
    ...
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        ...
        const { worker_id } = await inScope(t.adminSession, () =>
          createWorker({ full_name: 'Alice', session: t.adminSession }),
        );
        ...
        const result = await inScope(ownerSession, () =>
          editWorker({
            worker_id,
        ...
        const [w] = await inScope(ownerSession, () =>
  ...
  it('owner cannot edit admin-only field full_name', async () => {
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        const t = await seedTenant(pool);
        const { worker_id } = await inScope(t.adminSession, () =>
          createWorker({ full_name: 'Bob', session: t.adminSession }),
        );
  ...
  it('manager admin edits full_name + phone: two history rows, event fields contains both', async () => {
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        const t = await seedTenant(pool);
        const { worker_id } = await inScope(t.adminSession, () =>
          createWorker({ full_name: 'Carol', session: t.adminSession }),
        );
  ...
  it('viewer (not owner) cannot edit phone', async () => {
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        const t = await seedTenant(pool);
        const { worker_id } = await inScope(t.adminSession, () =>
          createWorker({ full_name: 'Dave', session: t.adminSession }),
        );
  ...
  it('unchanged value produces no history row and no event', async () => {
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        const t = await seedTenant(pool);
        const { worker_id } = await inScope(t.adminSession, () =>
          createWorker({ full_name: 'Eve', session: t.adminSession }),
        );
  ...
  it('stale expected_version throws CONFLICT', async () => {
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        const t = await seedTenant(pool);
        const { worker_id } = await inScope(t.adminSession, () =>
          createWorker({ full_name: 'Frank', session: t.adminSession }),
        );
  ...
  it('admin edits job_title + org_unit_id: two history rows, event fields includes both, worker row updated', async () => {
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        const t = await seedTenant(pool);
        const { worker_id: head } = await inScope(t.adminSession, () =>
          createWorker({
            full_name: 'Manager',
            session: t.adminSession,
          }),
        ...
        const { worker_id } = await inScope(t.adminSession, () =>
          createWorker({ full_name: 'Helen', session: t.adminSession }),
        );
  ...
  it('cross-tenant worker_id returns NOT_FOUND', async () => {
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        ...
        const { worker_id } = await inScope(t1.adminSession, () =>
          createWorker({ full_name: 'Grace', session: t1.adminSession }),
        );

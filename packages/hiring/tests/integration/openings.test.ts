  ...
  it('adds an opening with next seq and closes it with a reason', async () => {
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        ...
        const added = await inScope(t.adminSession, () =>
          addOpening({ requisition_id, input: {}, session: t.adminSession }),
        );
        expect(added.seq).toBe(2);
        expect(await countEvents(pool, t.tenant_id, 'hiring.opening.opened')).toBe(2);

        const closed = await inScope(t.adminSession, () =>
          closeOpening({
        ...
        const [o] = await inScope(t.adminSession, () =>
          hiringDb().select().from(opening).where(eq(opening.id, added.opening_id)),
        );

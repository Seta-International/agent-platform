  ...
  it('upload sets thread_id + origin=chat and returns a presigned url', async () => {
    await withTestDb(dbEnv(), async ({ pool, databaseUrl }) => {
      ...
      try {
        ...
        // fallback) — this only opens the executor context the domain fns require.
    ...
    await withTestDb(dbEnv(), async ({ pool, databaseUrl }) => {
      ...
      try {
        ...
        await scoped(tenant_id, async () => {
          ...
          let remaining = await pool.query(`SELECT 1 FROM knowledge.files WHERE id = $1`, [
            file_id,
          ]);

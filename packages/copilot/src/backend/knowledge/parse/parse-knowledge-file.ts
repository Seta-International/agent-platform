...
/**
 * Idempotently create the per-tenant LIST partition for copilot.tenant_knowledge_chunks.
...
async function ensureChunksPartition(pool: Pool, tenantId: string): Promise<void> {
  ...
  const digest = createHash('sha256')
    .update(`copilot.tenant_knowledge_chunks|${tenantId}`)
    .digest();
...
export async function parseKnowledgeFile(
  ...
  try {
    const fileRow = await deps.pool.query<{ s3_key: string; filename: string; status: string }>(
      `SELECT s3_key, filename, status FROM copilot.tenant_knowledge_files

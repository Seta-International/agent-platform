...
function instructionsText(cap: number): string {
  return [
    ...
    'to them" / "assign to him" right after candidates were listed): call proposeAssignment(taskId)',
    ...
    'been listed yet (e.g. "assign the AWS inventory task to someone"), FIRST call callTaskAnalyzer',
    'with intent=find_tasks and that name as the query, THEN call proposeAssignment with the',
    'matching task ("first" when it is the top result). proposeAssignment runs the whole recommend',
    'pipeline itself and pauses for the user to confirm the assignment; do NOT call callSkillMatcher/',
    'callAvaiChecker/callRecommender yourself for a single-task recommend. "Assign" is NEVER a direct',
    'write you perform — it ALWAYS goes through proposeAssignment, which asks the user to confirm.',
    '',

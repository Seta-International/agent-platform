    ...
    await withTestDb(
      ...
      async ({ pool, databaseUrl }) => {
        ...
        try {
          ...
          // Each concurrent call gets its own scoped() context (as two real concurrent
          // requests would) — sharing one scoped() would pin both calls to the same
          // connection and serialize what this test means to race.
      ...
      async ({ pool, databaseUrl }) => {
        ...
        try {
          ...
          await scoped(crypto.randomUUID(), async () => {
            ...
            await expect(
              setTenantEmailDomains(
                { tenant_id: tenantId, email_domains: ['acme.com'] },
                userActor,
              ),
            ).rejects.toSatisfy(
              (e: unknown) => e instanceof IdentityError && e.code === 'FORBIDDEN',
            );
          });
        } finally {
          resetCoreDb();
      ...
      async ({ pool, databaseUrl }) => {
        ...
        try {
          ...
          await scoped(crypto.randomUUID(), async () => {
            ...
            await expect(
              setTenantEmailDomains(
                { tenant_id: tenantId, email_domains: ['evil.com'] },
                CLI_ACTOR,
              ),
            ).rejects.toSatisfy(

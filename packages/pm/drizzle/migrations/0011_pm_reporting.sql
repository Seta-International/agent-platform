...
CREATE TABLE "pm"."flag" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "flag_category_check" CHECK (category IN ('quality', 'cost', 'delivery', 'performance')),
...
CREATE TABLE "pm"."norm_baseline" (
	...
	"goal_threshold" numeric(18, 6),
	"yellow_threshold" numeric(18, 6),
	"formula_ref" text,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "norm_baseline_category_check" CHECK (category IN ('quality', 'cost', 'delivery', 'performance')),
	CONSTRAINT "norm_baseline_direction_check" CHECK (direction IN ('higher_better', 'lower_better'))
);
--> statement-breakpoint
CREATE TABLE "pm"."norm_snapshot" (
	...
	"formula_ref" text,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "norm_snapshot_category_check" CHECK (category IN ('quality', 'cost', 'delivery', 'performance')),
	CONSTRAINT "norm_snapshot_direction_check" CHECK (direction IN ('higher_better', 'lower_better'))
);
--> statement-breakpoint
CREATE TABLE "pm"."project_week_rollup" (
	...
	CONSTRAINT "project_week_rollup_cost_colour_check" CHECK (cost_colour IN ('green', 'yellow', 'red', 'gray')),
	CONSTRAINT "project_week_rollup_delivery_colour_check" CHECK (delivery_colour IN ('green', 'yellow', 'red', 'gray')),
	CONSTRAINT "project_week_rollup_performance_colour_check" CHECK (performance_colour IN ('green', 'yellow', 'red', 'gray')),
...
ALTER TABLE "pm"."flag" ADD CONSTRAINT "flag_report_id_report_id_fk" FOREIGN KEY ("report_id") REFERENCES "pm"."report"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "pm"."project_week_rollup" ADD CONSTRAINT "project_week_rollup_project_id_project_id_fk" FOREIGN KEY ("project_id") REFERENCES "pm"."project"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "pm"."report" ADD CONSTRAINT "report_project_id_project_id_fk" FOREIGN KEY ("project_id") REFERENCES "pm"."project"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
CREATE INDEX "comment_by_report" ON "pm"."comment" USING btree ("tenant_id","report_id","created_at");--> statement-breakpoint
CREATE UNIQUE INDEX "flag_report_category_uniq" ON "pm"."flag" USING btree ("tenant_id","report_id","category");--> statement-breakpoint
...
CREATE UNIQUE INDEX "norm_baseline_version_uniq" ON "pm"."norm_baseline" USING btree ("tenant_id","metric_id","catalog_version");--> statement-breakpoint
...
CREATE UNIQUE INDEX "project_week_rollup_uniq" ON "pm"."project_week_rollup" USING btree ("tenant_id","project_id","week_start");--> statement-breakpoint
CREATE UNIQUE INDEX "report_identity_uniq" ON "pm"."report" USING btree ("tenant_id","project_id","week_start","reporter_id");--> statement-breakpoint
CREATE INDEX "report_by_project_week" ON "pm"."report" USING btree ("tenant_id","project_id","week_start");--> statement-breakpoint

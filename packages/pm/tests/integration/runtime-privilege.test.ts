...
async function seedProject(pool: import('pg').Pool, tenantId: string, name: string) {
  const accountResult = await pool.query<{ id: string }>(
    `INSERT INTO pm.account (tenant_id, name) VALUES ($1, $2) RETURNING id`,
    [tenantId, `${name}-account`],
  );
  await pool.query(`INSERT INTO pm.project (tenant_id, account_id, name) VALUES ($1, $2, $3)`, [
    tenantId,
...
describe('pm runtime privilege: pmDb() resolves through the executor', () => {
  it('scoped(tenantA) sees only tenant A rows through pmDb(); maintenance() sees both', async () => {
    await withTestDb(
      { templateDbName: env.template(), baseUrl: env.base() },
      async ({ pool, databaseUrl }) => {
        resetPmDb();
        await seedProject(pool, TENANT_A, 'project-a');
        await seedProject(pool, TENANT_B, 'project-b');
        ...
        await pool.query(`GRANT SELECT ON pm.account, pm.project TO seta_app`);
        ...
        try {
          ...
          const scopedRows = await scoped(TENANT_A, async () =>
            pmDb().select({ name: project.name }).from(project),
          );
          expect(scopedRows.map((r) => r.name)).toEqual(['project-a']);
          ...
          // app pool), so pmDb()'s cache must be keyed on Pool identity — not
          ...
          expect(adminRows.map((r) => r.name).sort()).toEqual(['project-a', 'project-b']);
        } finally {
          resetPmDb();
          await closePools();
        }

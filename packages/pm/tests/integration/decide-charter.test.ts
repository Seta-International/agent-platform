  ...
  it('BoD approve from pmo_approved creates the project + owner grant + both events', async () => {
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        ...
        const { charter_id } = await seedCharter(pool, t.adminSession, t.tenant_id);
        await inScope(pmo, () => pmoSignOffCharter({ charter_id, session: pmo }));
        const { project_id } = await inScope(bod, () =>
          bodApproveCharter({ charter_id, session: bod }),
        );

        const [p] = await inScope(t.adminSession, () =>
          pmDb().select().from(project).where(eq(project.id, project_id)),
        );
        ...
        const grants = await inScope(t.adminSession, () =>
          pmDb().select().from(projectAccess).where(eq(projectAccess.project_id, project_id)),
        );
  ...
  it('BoD approve before PMO sign-off is a CONFLICT', async () => {
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        ...
        const { charter_id } = await seedCharter(pool, t.adminSession, t.tenant_id);
        await expect(
          inScope(bod, () => bodApproveCharter({ charter_id, session: bod })),
  ...
  it('PMO cannot BoD-approve and BoD cannot PMO-sign-off (403)', async () => {
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        ...
        const { charter_id } = await seedCharter(pool, t.adminSession, t.tenant_id);
        await expect(
          inScope(pmo, () => bodApproveCharter({ charter_id, session: pmo })),
        ).rejects.toMatchObject({
          code: 'FORBIDDEN',
        });
        await inScope(pmo, () => pmoSignOffCharter({ charter_id, session: pmo }));
        await expect(
          inScope(bod, () => pmoSignOffCharter({ charter_id, session: bod })),
        ).rejects.toMatchObject({
          code: 'FORBIDDEN',
        });
      } finally {
  ...
  it('viewer cannot sign off; stale version conflicts at BoD', async () => {
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        ...
        await expect(
          inScope(viewer, () => pmoSignOffCharter({ charter_id, session: viewer })),
        ).rejects.toThrow(/permission/i);
        await inScope(pmo, () => pmoSignOffCharter({ charter_id, session: pmo }));
        await expect(
          inScope(bod, () => bodApproveCharter({ charter_id, expected_version: 99, session: bod })),
  ...
  it('reject at submitted records rejected_stage=pmo; reject at pmo_approved records bod', async () => {
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        ...
        await inScope(pmo, () =>
          rejectCharter({ charter_id: a.charter_id, reason: 'no capacity', session: pmo }),
        );
        const [ca] = await inScope(t.adminSession, () =>
          pmDb().select().from(charter).where(eq(charter.id, a.charter_id)),
        );
        ...
        await inScope(pmo, () => pmoSignOffCharter({ charter_id: b.charter_id, session: pmo }));
        await inScope(bod, () =>
          rejectCharter({ charter_id: b.charter_id, reason: 'budget window', session: bod }),
        );
        const [cb] = await inScope(t.adminSession, () =>
          pmDb().select().from(charter).where(eq(charter.id, b.charter_id)),
        );
  ...
  it('BoD cannot reject a still-submitted charter (403)', async () => {
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        ...
        const { charter_id } = await seedCharter(pool, t.adminSession, t.tenant_id);
        await expect(
          inScope(bod, () => rejectCharter({ charter_id, reason: 'x', session: bod })),
  ...
  it('(G) BoD approve notifies the submitter (distinct from approver)', async () => {
    await withTestDb(ctx, async ({ pool, databaseUrl }) => {
      ...
      try {
        ...
        const { charter_id } = await seedCharter(pool, t.adminSession, t.tenant_id);
        await inScope(pmo, () => pmoSignOffCharter({ charter_id, session: pmo }));
        await inScope(bod, () => bodApproveCharter({ charter_id, session: bod }));

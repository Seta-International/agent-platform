...
export interface ListTasksBySkillTagInput {
  ...
  tags: string[];
  /** Exact percent_complete to match (0 | 50 | 100). Omit for no status filter. */
...
export async function listTasksBySkillTag(
  ...
  // Build ARRAY[...] literal inline (escape ') so postgres receives an explicit
  // text[] value rather than a scalar param, mirroring list-tasks.ts:277-280.

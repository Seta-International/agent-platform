...
import { plannerGetBoardSnapshotTool } from '@seta/planner/agent-tools';
import {
  buildRegistry,
...
describe('planner_getBoardSnapshot tool', () => {
  it('returns plan meta, buckets, and status counts', async () => {
    await withAgentTestDb(async ({ pool }) => {
      ...
      const res = (await plannerGetBoardSnapshotTool.execute!(
        { planId: plan.id },
        makeToolContext({ user_id: admin_user_id, tenant_id }),
      )) as {
        plan: { planId: string; name: string; groupId: string };
        buckets: Array<{ id: string; name: string }>;
        counts: { notStarted: number; inProgress: number; completed: number; total: number };
      };

      expect(res.plan.planId).toBe(plan.id);
      expect(res.plan.name).toBe('Sprint');
      expect(res.plan.groupId).toBe(group.id);
      expect(res.counts.notStarted).toBeGreaterThanOrEqual(1);
      expect(res.counts.total).toBeGreaterThanOrEqual(1);
    });
  });

  it('is registered with permission planner.reporting.read', () => {
    expect(requiredPermissionFor(plannerGetBoardSnapshotTool)).toBe('planner.reporting.read');
  });
});

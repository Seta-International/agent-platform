...
import { plannerGetWorkloadTool } from '@seta/planner/agent-tools';
import {
  buildRegistry,
...
describe('planner_getWorkload tool', () => {
  it('returns per-person open-task counts for a group', async () => {
    await withAgentTestDb(async ({ pool }) => {
      ...
      const res = (await plannerGetWorkloadTool.execute!(
        { groupId: group.id },
        makeToolContext({ user_id: admin_user_id, tenant_id }),
      )) as { rows: Array<{ userId: string; displayName: string; openTaskCount: number }> };

      expect(res.rows.length).toBeGreaterThan(0);
      expect(res.rows[0]!.openTaskCount).toBe(1);
    });
  });

  it('is registered with permission planner.reporting.read', () => {
    expect(requiredPermissionFor(plannerGetWorkloadTool)).toBe('planner.reporting.read');
  });
});

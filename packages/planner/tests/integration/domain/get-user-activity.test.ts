...
import { createGroup, createPlan, createTask, getUserActivity } from '@seta/planner';
import {
  buildRegistry,
...
describe('getUserActivity', () => {
  it("returns a user's recent planner events within the caller's groups", async () => {
    await withAgentTestDb(async ({ pool }) => {
      ...
      const res = await getUserActivity({ user_id: admin_user_id, session, limit: 20 });
  ...
  it('returns empty events for a user with no activity', async () => {
    await withAgentTestDb(async ({ pool }) => {
      const { tenant_id, admin_user_id } = await createTestTenantWithAdmin({ pool });
      const session = buildAdminSession({
        tenant_id,
      ...
      const res = await getUserActivity({ user_id: otherUserId, session, limit: 20 });

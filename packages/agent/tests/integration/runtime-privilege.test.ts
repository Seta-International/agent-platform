...
import { agentDb, resetAgentDb } from '../../src/backend/db/index.ts';
...
async function seedRun(pool: import('pg').Pool, tenantId: string, workflowId: string) {
  await pool.query(
    `INSERT INTO agent.workflow_runs
       (run_id, workflow_id, tenant_id, started_by, started_via, input_summary, status)
     VALUES (gen_random_uuid(), $1, $2, gen_random_uuid(), 'chat', '{}'::jsonb, 'running')`,
    [workflowId, tenantId],
  );
}

describe('agent runtime privilege: agentDb() resolves through the executor', () => {
  it('scoped(tenantA) sees only tenant A rows through agentDb(); maintenance() sees both', async () => {
    await withTestDb(
      { templateDbName: env.template(), baseUrl: env.base() },
      async ({ pool, databaseUrl }) => {
        resetAgentDb();
        await seedRun(pool, TENANT_A, 'wf-a');
        await seedRun(pool, TENANT_B, 'wf-b');
        ...
        await pool.query(`GRANT SELECT ON agent.workflow_runs TO seta_app`);
        ...
        try {
          ...
          const scopedRows = await scoped(TENANT_A, async () =>
            agentDb().select({ workflowId: workflowRuns.workflowId }).from(workflowRuns),
          );
          expect(scopedRows.map((r) => r.workflowId)).toEqual(['wf-a']);
          ...
          // app pool), so agentDb()'s cache must be keyed on Pool identity — not just
          // "have we built one yet" — or this read would silently run back through
          // the scoped(tenantA) app-pool connection and still see one row.
          const adminRows = await maintenance(async () =>
            agentDb().select({ workflowId: workflowRuns.workflowId }).from(workflowRuns),
          );
          expect(adminRows.map((r) => r.workflowId).sort()).toEqual(['wf-a', 'wf-b']);
        } finally {
          resetAgentDb();
          await closePools();
        }

 ...
 *     run).
 *   • Chat-flow `__chat_hitl:planner_proposeAssignment` runs (transitional) —
 *     the synthetic workflow_runs row carries `{tool_id, thread_id}` only, so
 *     the taskId is reachable only through `workflow_approvals.proposed_payload`.
 *     Both rows are inserted atomically by insert-chat-hitl-approval.ts.
 ...
 *     `proposed_payload`. We match via the approval JOIN (mirroring the chat
 *     branch) so the row is only a mutex hit once its approval card is readable.
 */
  ...
  const result = await db.execute(sql`
    SELECT run_id FROM (
      ...
      SELECT r.run_id, r.started_at
       ...
       WHERE r.workflow_id IN (${CHAT_HITL_PROPOSE_ID}, ${STAFFING_ORCHESTRATOR_ID})
         AND r.status IN ('running', 'paused')
         AND r.tenant_id = ${opts.tenantId}
